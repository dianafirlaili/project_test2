                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('penilaian.cari') }}" method="GET" class="d-inline">
                        <div class="row mt-0">
                            ...
                            <div class="col-4 col-md-2">
                                <button class="btn btn-info ml-0 mt-0 mt-sm-0 px-4 py-1 " type="submit" value="Cari">
                                    <span class="pcoded-micon"><i class="fas fa-search"></i> Cari </button>
                            ...
                            @forelse ($datas as $data)
                                <tr id="sid{{ $data->id }}">
                                    ...
                                    <td data-toggle="modal" data-target="#pilihguru{{ $data->id }}">
                                        <label for=""
                                            class="text-{{ $data->guru != null ? 'dark' : 'danger' }}">{{ $data->guru != null ? $data->guru->nama : 'Data tidak ditemukan' }}</label>
                                    </td>
...
@section('containermodal')
    @forelse ($datas as $data)
        <div class="modal fade" id="pilihguru{{ $data->id }}" tabindex="-1" role="dialog"
            aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <form method="post" action="{{ route('store.pengajar', $data->id) }}" enctype="multipart/form-data">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">Pilih Guru Pengajar {{ $data->tingkatan }}
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <select
                                    class="js-example-basic-single form-control-sm @error('guru_id')
                            is-invalid
                        @enderror"
                                    name="guru_id" style="width: 75%" required>
                                    @if ($data->guru != null)
